    ...
    fn do_matches<'a>(&self, matches: &ArgMatches<'a>) {
        if let Some(ref matches) = matches.subcommand_matches("beer_style") {
            ...
            if !og.is_none() {
                ...
                resp.retain(|&style|og_value > style.original_gravity_min && og_value < style.original_gravity_max);
            }

            if !fg.is_none() {
                let fg_value = fg.unwrap().parse::<f32>().unwrap();
                resp.retain(|&style|fg_value > style.final_gravity_min && fg_value < style.final_gravity_max);
            }
            ...
            if !srm.is_none() {
                ...
                resp.retain(|&style|srm_value > style.color_srm_min && srm_value < style.color_srm_max);
            }
            ...
            for x in &resp {
                println!("* {}", x.name);
                println!("    OG: {}-{}, FG: {}-{}, ABV: {}%-{}%, IBU: {}-{}, SRM: {}-{}",
                         x.original_gravity_min, x.original_gravity_max,
                         x.final_gravity_min, x.final_gravity_max,
                         x.abv_min, x.abv_max,
                         x.ibu_min, x.ibu_max,
                         x.color_srm_min, x.color_srm_max);
            }
        }
    }
}
